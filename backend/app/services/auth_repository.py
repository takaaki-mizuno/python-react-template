...
class AuthRepository(AuthRepositoryInterface):
    ...
    def __init__(self, session_factory):
        ...
        self._transaction_session: ContextVar[
            AsyncSession | None] = ContextVar(
                "auth_repository_transaction_session", default=None)

    @asynccontextmanager
    async def transaction(self) -> AsyncIterator[None]:
        if self._transaction_session.get() is not None:
            yield
            return
    ...
    async def _session_scope(self) -> AsyncIterator[AsyncSession]:
        transaction_session = self._transaction_session.get()
    ...
    async def _persist(self, session: AsyncSession) -> None:
        if session is self._transaction_session.get():
            await session.flush()
        else:
    ...
    async def create_user(self, email: str, password_hash: str) -> User:
        async with self._session_scope() as session:
            ...
            try:
                ...
                if session is not self._transaction_session.get():
